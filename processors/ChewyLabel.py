...
def convert_xls_data(uploaded_file, dest_file):
    ...
    for i in range(21, 21 + column_length):  # Loop through rows in column B in the upload file
        ...
        try:
            numeric_value = float(value_from_B)  # Try to convert the value to a float
            # Sum up QTY from column B
            QTY_total += numeric_value
            print(f"Adding {numeric_value} to QTY_total")  # Debugging print for QTY
        except (ValueError, TypeError):
            print(f"Non-numeric value found in B{i}: {value_from_B}")  # Handle non-numeric values
